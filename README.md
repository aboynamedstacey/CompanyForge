...
6. **Calibrate with your knowledge.** The AI resolves defaults it can answer from established sources (published standards, documented conventions). Then it presents only the open questions for your input, the things that require practitioner judgment rather than things the AI already knows.
...
**Infrastructure is conditional.** Not every domain needs document processing, execution modes, or formal iteration protocols. The forge evaluates what the domain requires and skips what it doesn't. Ten files that all earn their place beat seventeen where four are filler.
...
Each iteration refined the forge. Build a domain, see what the forge gets wrong, fix the forge, build the next domain better.
